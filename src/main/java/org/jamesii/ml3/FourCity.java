...
public class FourCity implements IInitialStateBuilder {
    public IState buildInitialState(Model model, IStateFactory sf, IAgentFactory af, RandomGenerator rng, Parameters par) {
        ...
        IAgent mid = af.createAgent(location, 0);

        IAgent linkMid = af.createAgent(link, 0);
        ...
        mid.setAttributeValue("type", new StringValue("std"));
        mid.setAttributeValue("quality", new RealValue(.1));
        mid.setAttributeValue("resources", new RealValue(.1));
        mid.setAttributeValue("x", new RealValue(0.5));
        mid.setAttributeValue("y", new RealValue(0.5));

        link1.setAttributeValue("type", new StringValue("fast"));
        double dist1 = Math.sqrt(sqDist(mid, exit1));
        ...
        double dist2 = Math.sqrt(sqDist(mid, exit2));
        ...
        linkMid.setAttributeValue("type", new StringValue("fast"));
        double distMid = Math.sqrt(sqDist(entry, mid));
        linkMid.setAttributeValue("distance", new RealValue(distMid));
        double frictMid = frict(distMid, par, rng);
        linkMid.setAttributeValue("friction", new RealValue(frictMid));

        entry.addLink("links", linkMid);
        linkMid.addLink("endpoints", entry);

        mid.addLink("links", linkMid);
        linkMid.addLink("endpoints", mid);

        mid.addLink("links", link1);
        link1.addLink("endpoints", mid);

        mid.addLink("links", link2);
        link2.addLink("endpoints", mid);
